...
public class RepositorySortTest {
	...
	public void testSort() {
		assertArrayEquals(allEmployes,RepositoryTest.toArray( repository.sorted(EmployeeComporators.getComporator(EmployeeComporators.ID)).getAll()));
		assertArrayEquals(employeesOrderByName, RepositoryTest.toArray(repository.sorted(EmployeeComporators.getComporator(EmployeeComporators.NAME)).getAll()));
		assertArrayEquals(employeesOrderBySalary, RepositoryTest.toArray(repository.sorted(EmployeeComporators.getComporator(EmployeeComporators.SALARY)).getAll()));
		assertArrayEquals(employeesOrderByCompany, RepositoryTest.toArray(repository.sorted(EmployeeComporators.getComporator(EmployeeComporators.COMPANY)).getAll()));
	}
	
	@Test
	public void testSortByString() {
		assertArrayEquals(allEmployes,RepositoryTest.toArray( repository.sorted(EmployeeComporators.getComporator("Id")).getAll()));
		assertArrayEquals(employeesOrderByName, RepositoryTest.toArray(repository.sorted(EmployeeComporators.getComporator("name")).getAll()));
		assertArrayEquals(employeesOrderBySalary,RepositoryTest.toArray( repository.sorted(EmployeeComporators.getComporator("sALAry")).getAll()));
		assertArrayEquals(employeesOrderByCompany,RepositoryTest.toArray( repository.sorted(EmployeeComporators.getComporator("COMPANY")).getAll()));
	}
	
	@Test
	public void testFilterAndSort() {
		assertArrayEquals(new Employee[]{KATS,ADMINOV},
				RepositoryTest.toArray(repository.filteredByCompany(COMPANY2).sorted(EmployeeComporators.getComporator("Id")).getAll()));
		assertArrayEquals(new Employee[]{ADMINOV,KATS},
				RepositoryTest.toArray(repository.filteredByCompany(COMPANY2).sorted(EmployeeComporators.getComporator("Salary")).getAll()));
		assertArrayEquals(new Employee[]{KATS,IVANOV},
				RepositoryTest.toArray(repository.filteredBySalary(25000, 42000).sorted(EmployeeComporators.getComporator("Company")).getAll()));
